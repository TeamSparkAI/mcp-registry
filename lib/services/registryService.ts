...
export class RegistryServiceImpl implements RegistryService {
    ...
    private async shouldReloadRegistry(): Promise<boolean> {
        try {
            ...
            console.debug('Error checking registry file timestamp:', error);
    ...
    private async loadRegistry(): Promise<void> {
        ...
        if (await this.loadFromFile()) {
            console.debug('Loaded server registry from local file');
            this.lastLoadTime = Date.now();
            return;
        }

        // Fallback to remote fetch
        console.debug('Loading server registry from', MCP_REGISTRY_API_URL);
        try {
            ...
            console.debug('Loaded', this.servers.length, 'servers from MCP registry');
            this.lastLoadTime = Date.now();
            
            // Generate the local registry file
            await this.generateRegistryFile();
        } catch (error) {
            console.error('Error loading server registry:', error);
            this.servers = [];
    ...
    private async loadFromFile(): Promise<boolean> {
        try {
            ...
            console.debug('Error loading registry from file:', error);
            return false;
        }
    }
    ...
    private async fetchAllServers(): Promise<{ servers: ServerJSON[], metadata: { count: number; next_cursor?: string } }> {
        ...
        while (true) {
            ...
            console.debug(`Fetched ${data.servers?.length || 0} servers (total so far: ${allServers.length}, total available: ${totalCount})`);
        }
    ...
    async generateRegistryFile(): Promise<void> {
        try {
            ...
            console.debug(`Generated server-registry.json with ${this.servers.length} servers at ${REGISTRY_FILE_PATH}`);
        } catch (error) {
...
export function getRegistryService(): RegistryService {
    if (!global.registryServiceInstance) {
        console.debug('[getRegistryService] Creating new singleton instance');
        global.registryServiceInstance = new RegistryServiceImpl();
    } else {
        console.debug('[getRegistryService] Returning existing singleton instance');
    }
